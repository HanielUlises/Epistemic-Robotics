...
The action node does not determine the outcome, which is what distinguishes the
observation from a scripted one.
...
When the destination is unreachable the drive action selects a
\emph{frontier}: a cell that is known free, admits the robot and the planner's
inflation radius, and has unobserved cells within sensor range. It requests a
route to that cell, drives it, and re-issues the original query against the
enlarged map. Three properties of the selection are required for termination,
and each was established by a run that failed without it:
...
\paragraph{The branch the first three runs do not reach} Each of them follows
the same leaf: $r_1$ inspects bay 2, the pallet is present, and it is lifted.
That trace is observationally indistinguishable from a fixed sequence, so it
does not by itself establish that the plan is conditional. The branch structure
is stated in the narration rather than exhibited by the robot.
...
$r_1$ lift it, without leaving receiving. No preference for this solution is
encoded in the domain: a policy in which $r_2$ travels to a bay and inspects it
also satisfies the goal, and is longer.
...
the domain does not say. The separation also has to be made explicitly, because
\code{slam\_toolbox} names its services relative to the node and its map topic
\emph{absolutely}: three mappers in three namespaces publish to the same
\code{/map} until that topic is remapped.
