...
The film is usually sped up, because most of a run is a robot crossing an
aisle. `--speed` divides the positions by that factor and leaves the hold
alone, so a caption over a three-times film is still on screen long enough to
read.

A recording of this mission without them is a video of a robot fetching a
pallet. Everything that makes it worth watching -- which aisle the fleet is
uncertain about, what the inspection settled for the robot that looked and
withheld from the one that did not, and the moment the lift told r2 an answer
it never went to find -- happens in the model and is invisible on the floor.
These are the lines that say so, placed at the second the log says they
happened.
...
DISPATCH = re.compile(
    ...
    r'(?P<action>goto_zone|deploy|look_into|shut_valve|pick_up|drop_off): '
...
ZONE = {
    'bay2': 'aisle 2',
    'bay3': 'aisle 3',
    'lane': 'the service lane',
    'dock_south': 'shipping',
    'dock_north': 'receiving',
    'corridor': 'the west corridor',
}

DISPATCH_CAPTION = {
    'goto_zone': (
        'Driving to {place}',
        "The planner's {agent} is RMF's {robot}; Open-RMF owns the route, the "
        'floor space and the negotiation'),
    'look_into': (
        'Inspecting {place}',
        'Semi-private sensing: only the robot standing in the aisle sees the '
        'answer, and r2 sees that it looked'),
    'pick_up': (
        'Lifting the pallet in {place}',
        'Public, and its precondition names the aisle, so every agent watching '
        'learns which aisle held it'),
    'drop_off': (
        'Unloading at {place}',
        'The ontic half of the goal'),
}
...
def applied_caption(action, outcome, worlds, designated):
    ...
    if action.startswith('inspect'):
        if outcome and outcome.endswith('found'):
            return ('r1 now knows the pallet is here',
                    f'The designated worlds fall to {designated} of {worlds}: '
                    f'r1 has the answer and r2 still does not')
        if outcome and outcome.endswith('empty'):
            return ('r1 now knows this aisle is empty',
                    f'Two aisles, so elimination settles the other one: {model}')
        return ('Inspected', f'Sensing narrowed the model to {model}')

    if action.startswith('pickup'):
        return ('Lifted, and r2 learned which aisle without going',
                f'The precondition names the aisle and the action is public, so '
                f'the model collapses to {model}')

    if action.startswith('unload'):
        return ('Delivered', f'Both conjuncts of the goal now hold: {model}')

    if action.startswith('go'):
        return ('Arrived', f'Driving is public, so positions stay common '
                           f'knowledge: {model}')
...
def main():
    ...
    for stamp, kind, match in events:
        ...
        if kind == 'planned':
            ...
            detail = ('Two aisles, one pallet, and the plan branches on what '
                      'the inspection will turn out to say')
        elif kind == 'together':
            headline = f"{match.group('count')} robots, one action"
            ...
            detail = ('The pallet is at receiving and r2 knows which aisle held '
                      'it, having never left its charger')
    ...
    with open(args.out, 'w') as handle:
        ...
        for i, (start, end, headline, detail) in enumerate(placed):
            ...
            # drive across the floor does not leave the frame bare.
