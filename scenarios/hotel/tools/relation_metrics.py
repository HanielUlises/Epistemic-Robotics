...
def render(rows, columns, agents, title):
    ...
    height = HEAD_H + len(agents) * CELL_H + 2 * PAD + 66
    ...
    notes = [
        ...
        'classes counts the distinct sets of worlds it considers possible.',
        'A red cell marks a step at which the relation is no longer reflexive:',
        'the frame has left S5, and what the agent holds is belief, which may be false.',
    ]
    for line, text in enumerate(notes):
        out.append(f'  <text class="sub" x="{PAD}" y="{foot + line * 13}">{text}</text>')
    out.append('</svg>')
    return '\n'.join(out)
