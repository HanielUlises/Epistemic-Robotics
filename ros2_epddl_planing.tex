...
\subsection{A Practical PlanSys2 Example}

To ground the preceding architectural description, we present a concrete example of a PlanSys2 deployment for a logistics scenario in which a robot must transport packages between named locations. The PDDL domain defines types, predicates, and durative actions; the problem instance specifies objects, an initial state, and a goal; and the terminal session illustrates the runtime interaction with the system.

\subsubsection{PDDL Domain}

\begin{lstlisting}[language={}, caption={PDDL domain for a robot transport task compatible with PlanSys2.}, label=lst:pddldomain]
(define (domain robot-transport)
...
\subsubsection{PDDL Problem Instance}

\begin{lstlisting}[language={}, caption={PDDL problem instance for the robot transport scenario.}, label=lst:pddlproblem]
(define (problem transport-task-1)
  (:domain robot-transport)
...
\subsubsection{PlanSys2 Terminal Session}

Once the domain and problem are loaded into the running PlanSys2 system, a human operator or autonomous node can populate the world state via the terminal client and trigger planning. The following listing shows the full interaction sequence, the POPF solver output, and the executor's action dispatch log.

\begin{lstlisting}[language={}, caption={PlanSys2 terminal session: instance and predicate setup, goal submission, plan generation, and execution with replanning.}, label=lst:plansys2run]
$ ros2 run plansys2_terminal plansys2_terminal

> set instance robot1 robot
> set instance warehouse location
> set instance loading-bay location
> set instance storage location
> set instance parcel1 package
> set instance parcel2 package

> set predicate (robot-at robot1 warehouse)
> set predicate (package-at parcel1 warehouse)
> set predicate (package-at parcel2 warehouse)
> set predicate (free robot1)
> set predicate (connected warehouse loading-bay)
> set predicate (connected loading-bay warehouse)
> set predicate (connected loading-bay storage)
> set predicate (connected storage loading-bay)
...
[plansys2][PlannerManager] Calling POPF solver...
[plansys2][PlannerManager] Plan received:

  0.000: (pick-up robot1 parcel1 warehouse)  [5.000]
  5.001: (navigate robot1 warehouse loading-bay)  [10.000]
  5.001: (pick-up robot1 parcel2 warehouse)  [5.000]
 15.002: (drop-off robot1 parcel2 loading-bay)  [5.000]
 20.003: (navigate robot1 loading-bay storage)  [10.000]
 30.004: (drop-off robot1 parcel1 storage)  [5.000]
 35.005: (navigate robot1 storage warehouse)  [10.000]

[plansys2][Executor] Dispatching: pick-up robot1 parcel1 warehouse
[plansys2][Executor] Action SUCCEEDED: pick-up robot1 parcel1 warehouse
[plansys2][Executor] Dispatching: navigate robot1 warehouse loading-bay
[plansys2][Executor] Action SUCCEEDED: navigate robot1 warehouse loading-bay
[plansys2][Executor] Dispatching: drop-off robot1 parcel2 loading-bay
[plansys2][Executor] Action ABORTED: drop-off robot1 parcel2 loading-bay
[plansys2][Executor] Precondition failed: (carrying robot1 parcel2) not satisfied
[plansys2][PlannerManager] Replanning triggered...
[plansys2][PlannerManager] New plan received:

  0.000: (navigate robot1 loading-bay warehouse)  [10.000]
 10.001: (pick-up robot1 parcel2 warehouse)  [5.000]
 15.002: (navigate robot1 warehouse loading-bay)  [10.000]
 25.003: (drop-off robot1 parcel2 loading-bay)  [5.000]
 30.004: (navigate robot1 loading-bay storage)  [10.000]
 40.005: (drop-off robot1 parcel1 storage)  [5.000]
 45.006: (navigate robot1 storage warehouse)  [10.000]

[plansys2][Executor] Plan execution completed successfully.
[plansys2][Executor] Goal achieved.
\end{lstlisting}

This session illustrates the full PlanSys2 loop: instance population, predicate assertion, goal submission, temporal plan generation by POPF, action-by-action dispatch to ROS2 action servers, and reactive replanning triggered by a precondition failure at runtime. The replanning event occurs entirely within the classical PDDL paradigm---no epistemic state is maintained, and the failure is resolved by regenerating a classical plan from the updated predicate set.
...
To illustrate the structure of EPDDL concretely, we present a simplified domain and problem specification for a multi-robot sensing scenario. Two robots, \texttt{r1} and \texttt{r2}, operate in an environment with a single corridor whose blocked status is initially unknown to both. Robot \texttt{r1} can sense whether the corridor is blocked (a sensing action), and can announce its finding to \texttt{r2} (a public announcement action). The goal is for both robots to know whether the corridor is blocked.

\subsubsection{EPDDL Domain}

\begin{lstlisting}[language={}, caption={EPDDL domain for a two-robot sensing and announcement scenario.}, label=lst:epddldomain]
...
\begin{lstlisting}[language={}, caption={EPDDL problem instance using a finitary S5-theory to specify the initial epistemic state.}, label=lst:epddlproblem]
...
The \texttt{:epistemic} block in the \texttt{:init} section instantiates a finitary S5-theory: the formula \texttt{(C All (Kw r1 (blocked c1)) false)} asserts that it is common knowledge among all agents that robot \texttt{r1} does not currently know whether the corridor is blocked. The goal \texttt{([Kw. All] (blocked c1))} uses the group knowing-whether modality to require that all agents know whether the corridor is blocked upon plan completion. The key syntactic elements are summarized in Table~\ref{tab:epddlsyntax}.

\begin{table}[H]
\centering
\begin{tabular}{lll}
\toprule
\textbf{EPDDL Construct} & \textbf{Logical Meaning} & \textbf{Usage Context} \\
\midrule
\texttt{([i] phi)} & $\Box_i \varphi$ --- agent $i$ knows $\varphi$ & preconditions, goals \\
\texttt{(<i> phi)} & $\Diamond_i \varphi$ --- agent $i$ considers $\varphi$ possible & preconditions \\
\texttt{([Kw. i] phi)} & $\mathrm{Kw}_i \varphi$ --- agent $i$ knows whether $\varphi$ & goals, init \\
\texttt{([Kw. All] phi)} & $\mathrm{Kw}_\mathrm{All} \varphi$ --- all agents know whether $\varphi$ & goals \\
\texttt{(C All phi)} & $C_\mathrm{All} \varphi$ --- common knowledge of $\varphi$ & init, goals \\
...
\texttt{:sense-formula} & formula whose truth value is observed & sensing actions \\
...
\bottomrule
\end{tabular}
\caption{Key EPDDL syntactic constructs with their logical counterparts and usage contexts.}
...
This document has surveyed the state of the art in task planning for ROS2 multi-robot systems and the theoretical foundations of DEL-based epistemic planning, with the goal of establishing a preliminary research basis for the development of a DEL-based task planning system for multi-agent robotic platforms operating in uncertain environments.

We have established that ROS2 provides a mature and flexible middleware infrastructure for multi-robot systems, and that PlanSys2 offers a practical PDDL-based task planning framework that is already deployed in research and industrial contexts. However, PlanSys2 and the classical planning paradigm it implements are fundamentally limited in their ability to represent epistemic uncertainty, agent-relative knowledge states, and communication-as-action---all of which are essential for robust autonomous behavior in partially observable environments.

DEL and its associated planning framework provide the theoretical machinery to address these limitations. The product update semantics captures both physical and epistemic effects of actions in a unified formal model. The EPDDL language operationalizes this theory in a standardized, PDDL-compatible syntax, with a rich modality language, a flexible action type library system, and tool support through the \texttt{plank} toolkit.

The integration of these two pillars into a coherent robotic system architecture raises significant engineering and research challenges, particularly with respect to computational tractability, online replanning, and the perception-to-epistemic-state translation pipeline. These challenges define the research agenda for the next phase of work, which will involve prototype implementation, benchmark development using EPDDL, and experimental evaluation on multi-robot testbeds.

The convergence of formal epistemic reasoning with practical robotic middleware represents a promising and largely unexplored direction in the field of autonomous multi-robot systems. This preliminary research positions the subsequent work at the frontier of this convergence.
